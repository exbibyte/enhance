...
TEST_CASE( "RenderLight", "[ALL]" ) {
    SECTION( "RenderLight" ){
        ...
        render_light.AddDataSingle( RenderLightData::Coordinate(),  d_0 );
        render_light.AddDataSingle( RenderLightData::Lookat(),      d_1 );
        render_light.AddDataSingle( RenderLightData::Up(),          d_2 );
        render_light.AddDataSingle( RenderLightData::Perspective(), d_3 );
        render_light.AddDataSingle( RenderLightData::Ambient(),     d_4 );
        render_light.AddDataSingle( RenderLightData::Diffuse(),     d_5 );
        render_light.AddDataSingle( RenderLightData::Specular(),    d_6 );

        bool bRet = render_light.CheckIsValid();
        CHECK( true == bRet );

        RenderLightData::Ambient::Type data_ambient;
        ...
        CHECK( RenderLightData::Ambient::Size::value == iSizeAmbient );
        CHECK( RenderLightData::Diffuse::Size::value == iSizeDiffuse );
        CHECK( RenderLightData::Specular::Size::value == iSizeSpecular );
        CHECK( RenderLightData::Coordinate::Size::value == iSizeCoordinate );
