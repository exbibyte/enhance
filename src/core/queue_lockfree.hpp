//unbounded lock free queue, based on Art of Multiprocessor Programming section 10.5
#ifndef QUEUE_LF_H
#define QUEUE_LF_H
...
public:
    ...
    queue_lockfree();
    ~queue_lockfree();
    void enqueue( T & val ){ push(val); }
    bool dequeue( T & val ){ return pop(val); }
private:
    std::atomic< Node * > _head;
    std::atomic< Node * > _tail;
};
template< typename T >
queue_lockfree<T>::queue_lockfree(){
...
template< typename T >
void queue_lockfree<T>::push( T & val ){ //push item to the tail
...
template< typename T >
bool queue_lockfree<T>::pop( T & val ){ //obtain item from the head
    while( true ){
