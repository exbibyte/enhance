...
void fft::ifft_cooley_tukey( vector<complex<double> > & input, vector<double> & output ){
    //output real values
    pad_to_nearest_power_of_2( input );
    vector<complex<double> > temp = ifft_cooley_tukey_radix_2_dit( input );
    int n = temp.size();
    output.resize(n);
    for( int i = 0; i < n; ++i ){
...
std::vector<std::complex<double> > fft::ifft_cooley_tukey_radix_2_dit( std::vector<std::complex<double> > & input ){
    //performs radix-2 decimation in frequency algorithm
    //w = e^(2*pi*i/n)
    int n = input.size();
    if( 1 == n ){
