...
test_GLSceneManager:
	g++ -g -DGLFW_INCLUDE_GLCOREARB -std=c++0x $(src_folder_gl)/GLSceneManager.cpp $(src_folder_gl)/GLSLProgram.cpp $(src_folder_gl)/GLHelper.cpp $(src_folder_gl)/GLTexture.cpp $(src_folder_file)/textfile.cpp $(src_folder_file)/PPM.cpp $(src_folder_test_gl)/test_GLSceneManager.cpp -I/usr/local/include -I$(inc_folder_file) -I$(inc_folder_gl) -I$(inc_ui) -I$(inc_graph) -I$(inc_polymesh) -I$(inc_math) -pthread -lglfw3 -framework OpenGL -framework Cocoa -framework IOKit -framework CoreVideo -L/usr/local/lib -o $(build_dir)/test_GLSceneManager
