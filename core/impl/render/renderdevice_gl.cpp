...
bool renderdevice_gl_impl::process_init_window( renderdevice_gl_impl * context, ::e2::interface::i_renderpackage p ){
	...
	if( false == renderpayload_gl::unpack( p._num_payload, p._payload, payload ) )
	    return false;
	uint64_t * wind_dim = ( uint64_t * ) payload[0];
	assert( wind_dim );
...
bool renderdevice_gl_impl::process_exec_draw_batch( renderdevice_gl_impl * context, ::e2::interface::i_renderpackage p ){
    ...
    if( false == renderpayload_gl::unpack( p._num_payload, p._payload, payload ) )
	return false;
    uint64_t * primitive_type = ( uint64_t * )payload[0];
    uint64_t * offset = ( uint64_t * )payload[1];
    uint64_t * count = ( uint64_t * )payload[2];
    ...
    gl_count = static_cast<GLint>( *count );
