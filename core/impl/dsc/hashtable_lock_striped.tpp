...
template< class K, class V >
bool hashtable_lock_striped_impl< K, V >::add_hash_func( std::function< K ( K ) > hash_func ){
    _funcs_hash.push_back( hash_func );
    return true;
}
template< class K, class V >
bool hashtable_lock_striped_impl< K, V >::get_hash_func_current( std::function< K ( K ) > & hash_func ){
...
template< class K, class V >
bool hashtable_lock_striped_impl< K, V >::compute_hash( K const key, K & hashed_val ){
...
bool hashtable_lock_striped_impl< K, V >::set_default_hash_funcs( size_t const table_size ){
    hash_universal< K > h;
    return h.generate_hash_funcs( table_size, &_funcs_hash );
}
template< class K, class V >
