...
class DualQuaternionTest {
   ...
   public void transformTest() {
      ...
      for (int i=0; i<10; i++) {
         ...
         if (!trans.epsilonEquals(trans2, 1e-10)) {
            throw new NumericalException("Wrong transformation");
         }
         ...
         for (int j=0; j<10; j++) {
            ...
            double err = pr2.distance(pr1);
            if (err > 1e-15) {
               throw new NumericalException("Uh oh, transformation is incorrect");
            }      
            err = vr2.distance(vr1);
            if (err > 1e-15) {
               throw new NumericalException("Uh oh, transformation is incorrect");
            }
   ...
   public void dipTest(boolean printMe, int nSteps) {
      ...
      for (int i=0; i<nSteps; i++) {
         w[0] = 1-i*dt;
         w[1] = i*dt;
         qI.dualQuaternionIterativeBlending(w,qE, 1e-8, 5);         
      ...
      for (int i=0; i<nSteps; i++) {
         w[0] = 1-i*dt;
         w[1] = i*dt;
         qI.dualQuaternionLinearBlending(w,qE);         
         for (Point3d pnt : pnts) {
            qI.transform(out, pnt);
            if (printMe) {
               System.out.println(out);
   ...
   public static void main (String[] args) {
      DualQuaternionTest test = new DualQuaternionTest();
